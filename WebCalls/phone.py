...
def get_ask_book(given_args):
    base_url = parsed_phonebook[5]['base-url']
    function_name = parsed_phonebook[3]["sell-order-book"][0]
    result_parse_path = parsed_phonebook[3]["sell-order-book"][1]
    function_string = parsed_phonebook[1][function_name][0]
    arg_num = parsed_phonebook[1][function_name][1]
    args = parsed_phonebook[1][function_name][2]
    resp = ['arg required']
    if arg_num == 0:
        ...
        for arg in args:
            ...
            args_string += arg
            args_string += '='
            args_string += str(given_args[i])
            i += 1
        call_url = base_url + function_string + args_string
        resp = safe_request(call_url)
    if type(resp) == list:
        if resp[0] == 'arg required':
            sys.exit('unknown args required for api call')
...
def get_bid_book(given_args):
    base_url = parsed_phonebook[5]['base-url']
    function_name = parsed_phonebook[3]["buy-order-book"][0]
    result_parse_path = parsed_phonebook[3]["buy-order-book"][1]
    function_string = parsed_phonebook[1][function_name][0]
    arg_num = parsed_phonebook[1][function_name][1]
    args = parsed_phonebook[1][function_name][2]
    resp = ['arg required']
